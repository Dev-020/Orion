...
export default function ChatInterface({ session }) {
    ...
    useEffect(() => {
        ...
        let reconnectDelay = 2000; // Start with 2s delay
        ...
        const connectWebSocket = () => {
            ...
            socket.onerror = (err) => {
                ...
                socket.close(); // Ensure close triggers reconnect logic
            }
        ...
        return () => {
            shouldReconnect = false; // Prevent reconnect on unmount
            if (reconnectTimeout) clearTimeout(reconnectTimeout);
            if (ws.current) ws.current.close();
        }
    }, [session, token]) // Added token dependency to ensure re-connect if auth changes
